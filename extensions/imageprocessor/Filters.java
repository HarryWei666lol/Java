...
public class Filters {
	...
	//
	// NB: The 'return 0' and 'return new Color(0,0,0)' lines are simply place-holders
	...
	public static Color grayscale(Color c) {
		int grays= (c.getRed()+c.getGreen()+c.getBlue())/3;
		return new Color(grays, grays, grays);  // FIXME
	}
	...
	public static Color combineBrighter(Color c1, Color c2) {
		if((c1.getBlue()+c1.getGreen()+c1.getRed()) < (c2.getBlue()+c2.getGreen()+c2.getRed())) {
			return c2;
		}
		else {
			return c1;	
	...
	public static Color bgSubtract(Color source1Color, Color source2Color, int tolerance) {
		int blueDifference = Math.abs(source2Color.getBlue()-source1Color.getBlue());
		int greenDifference = Math.abs(source2Color.getGreen()-source1Color.getGreen());
		int redDifference = Math.abs(source2Color.getRed()- source1Color.getRed());
		if(redDifference < tolerance || greenDifference < tolerance || blueDifference < tolerance) {
			return Color.BLUE;
		}
		else {
			return source1Color;
